...
// add by chrono
char *
ngx_stream_lua_filter_by_lua_block(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    char        *rv;
    ngx_conf_t   save;
    ...
    save = *cf;
    cf->handler = ngx_stream_lua_filter_by_lua;
    cf->handler_conf = conf;
...
// add by chrono
char *
ngx_stream_lua_filter_by_lua(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    u_char                      *p;
    ngx_str_t                   *value;
    ...
    if (cmd->post == ngx_stream_lua_filter_inline) {
        /* Don't eval nginx variables for inline lua code */
        lscf->filter_src= value[1];
        ...
        *p = '\0';
        ...
        //if (lscf->body_filter_src.lengths == NULL) {
            /* no variable found */
            p = ngx_palloc(cf->pool, NGX_STREAM_LUA_FILE_KEY_LEN + 1);
            if (p == NULL) {
                return NGX_CONF_ERROR;
    ...
    // important!
    lscf->filter_handler = (ngx_stream_filter_pt) cmd->post;

    lmcf = ngx_stream_conf_get_module_main_conf(cf, ngx_stream_lua_module);

    lmcf->requires_filter = 1;
...
// add by chrono
char *
ngx_stream_lua_postread_by_lua_block(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    char        *rv;
    ngx_conf_t   save;
    ...
    save = *cf;
    cf->handler = ngx_stream_lua_postread_by_lua;
    cf->handler_conf = conf;
...
// add by chrono
char *
ngx_stream_lua_postread_by_lua(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    u_char                      *p, *chunkname;
    ngx_str_t                   *value;
    ngx_stream_lua_main_conf_t    *lmcf;
    ...
    if (cmd->post == ngx_stream_lua_postread_handler_inline) {
        chunkname = ngx_stream_lua_gen_chunk_name(cf, "postread_by_lua",
                                                sizeof("postread_by_lua") - 1);
        if (chunkname == NULL) {
            return NGX_CONF_ERROR;
        }

        lscf->postread_chunkname = chunkname;
    ...
    lscf->postread_handler = (ngx_stream_lua_handler_pt) cmd->post;
